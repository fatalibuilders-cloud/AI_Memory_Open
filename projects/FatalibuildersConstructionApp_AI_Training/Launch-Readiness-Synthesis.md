...
- **Swahili support (East Africa phase):** English/Swahili i18n with a header toggle — cookie-based locale + typed dictionaries (`src/i18n/`), no external i18n lib. **Full app coverage:** header, footer, homepage, pricing, paywall, account, login/signup, offline page, **the project wizard (all steps, field labels, select options, review, buttons) and the legal pages (terms/privacy/refund)**. Translated legal pages carry a governing-language note ("the English version prevails"). *(Only the cost-engine BOQ line-item labels and export files remain English — a small future batch.)*
