...
- **Diaspora "Verified Build" tier** (biggest revenue lever) — **now a paid, self-serve product.** The owner buys it through the same checkout rails (M-Pesa / Pesapal / card / mock); payments are **product-aware** (`purchases.product` + `project_id`), and completing a `verified_build` payment **creates the project's verification** (while `lifetime` still grants account access). Priced $99 / KES 12,900 (env-adjustable; Paddle uses `PADDLE_VERIFIED_PRICE_ID`). A public, unguessable **`/verify/[token]` trust page** shows project summary, estimated contract value, assigned supervisor and a milestone progress bar (Foundation → Handover). Admins advance status/milestones from `/admin`. The on-the-ground checking is still a **human service** the Fatali team performs — the paid checkout is the intake.
