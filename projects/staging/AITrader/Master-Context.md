...
- **Safe Mode's own daily target default is not yet set.** Aggressive Mode now defaults to 20%; Safe Mode should be meaningfully lower given its "safe" branding and lower trade frequency — recommend 1–5%/day, not yet a founder decision.
...
- **Trading Modes:**
  ...
  - **Aggressive Mode:** No win-probability filter; trades whenever conditions look suitable. Internal daily target: **20%** (founder-confirmed, 2026-07-14k — the top of the research-recommended 5–20% range). "Turn $50 into $1,000/day" / "$10 into $100/day" framing is retired in favor of this concrete, research-grounded number — **still not engineered toward via compounding/martingale lot-sizing, and never referenced in marketing or the MQL5 Market listing**, but now a real default for the EA's daily-profit-target setting rather than just narrative. Must be validated against real backtest/forward-test data before being trusted (see Open Questions) — it's a best-case target, not an expectation for every day.
  - Mode selection is user-configurable (a setting the customer picks, not something AITrader decides for them).
- **Daily Risk Controls:**
  - **Daily profit target (configurable):** user sets a daily profit goal; once equity gains reach it, the EA stops trading for the rest of the day. **Default: 20% in Aggressive Mode** (2026-07-14k); **Safe Mode's default is not yet set — should be meaningfully lower, recommend 1–5%/day pending founder decision** (see Open Questions).
