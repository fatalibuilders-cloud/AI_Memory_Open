...
**Progress note (2026-07-14u):** v1 signal replaced with v2 — Bollinger Bands + RSI + Stochastic mean-reversion scalping, restricted to forex/metals, per founder request for a genuine short-timeframe scalper. Flipped the Range Filter's logic (mean-reversion wants low ADX, not high) and re-derived the confidence heuristic to match. See `2026-07-14u_scalping_signal_v2.md`.
**Progress note (2026-07-14x):** every input renamed to plain English and grouped into 9 numbered sections in the EA's settings panel; added an `AUTO_TRADE`/`SIGNALS_ONLY` operation-mode choice — signals-only alerts the user with a suggested trade instead of placing it automatically. See `2026-07-14x_plain-english-inputs-and-manual-mode.md`.
**Progress note (2026-07-14y):** product renamed to **FatalibuildersTrader Scalper 1**; Aggressive Mode redesigned to risk/target a percentage of current equity per trade (15%/15% default) with its own daily loss ceiling, instead of small fixed-dollar amounts. Grounded in a Monte Carlo risk-of-ruin simulation. See `2026-07-14y_scalper1-name-and-aggressive-equity-risk.md`.
**Progress note (2026-07-14z):** narrowed to trade **XAUUSD and GBPUSD only** (was any forex/metals symbol); **M1 chart now enforced** (was recommended, not enforced); added a **session-open delay filter** — waits a configurable number of minutes (60 default) after each of the Asia/London/New York session opens (server time) before trading. Did not hardcode the "$0.30 profit" half of the request — existing profit-target inputs already support that value; changing Safe Mode's tuned defaults wasn't done without being asked. See `2026-07-14z_symbol-restriction-session-timing.md`.
**Progress note (2026-07-14zz):** founder confirmed the $0.30 Safe Mode profit target flagged above — implemented on both tiers, reintroducing a ~77%/~91% break-even win-rate requirement, documented rather than re-blocked. See `2026-07-14zz_safe-mode-profit-target-lowered-to-30-cents.md`.
**Progress note (2026-07-14zzz):** declined to decompile/combine 10 uploaded third-party `.ex5` files (compiled bytecode, and most named like commercial competitor products — an IP problem beyond the technical one). Pursued the real goal on our own source instead: renamed to **FatalibuildersTrader Super Scalpers**; `MaxTradesOpenAtOnce` raised from 2 to **20** (founder's explicit number, asked for directly since it multiplies simultaneous risk exposure); formalized a real gap found while answering "max trades in 5 minutes" — the entry signal was already re-checked every tick with no per-bar guard, now made an explicit, tunable feature (`AllowMultipleTradesPerSignal_Enabled`/`MaxTradesPerSignal_PerBar`). See `2026-07-14zzz_super-scalpers-rename-and-max-concurrency-raised.md`.
**Progress note (2026-07-15):** founder asked a second time to combine the uploaded `.ex5` files into a "super bot" — declined again, same reasoning. Pursued the goal with two independent, well-documented scalping techniques instead: a true trailing stop for `BREAKEVEN_AND_RUN` mode (keeps trailing behind price after breakeven is first locked, rather than sitting flat there) and a maximum trade duration force-close (default 15 minutes). See `2026-07-15_trailing-stop-and-max-duration-added.md`.
**Progress note (2026-07-16):** founder ran a real MT5 Strategy Tester backtest (GBPUSD, M1) and reported zero trades — first real backtest attempt on this project. Diagnosed from a shared screenshot and screen-recording (frames extracted via ffmpeg): the tester was applying a flat 50-point simulated spread throughout, which the EA's `MaxAllowedSpread_Points` filter (defaulted to 30) rejected on every tick, before the entry signal was ever evaluated. Raised the default to 60 to unblock this specific test — explicitly flagged as a tester-headroom number, not a validated real-account spread setting. See `2026-07-16_spread-filter-blocking-all-trades.md`. Backtest has not yet been re-run against the fix.
**Progress note (2026-07-16b):** founder re-ran the backtest after the spread fix — still zero trades. Found a second blocker in the same function: the quote-staleness check (`SymbolInfoInteger(_Symbol, SYMBOL_TIME)`) is known to behave unreliably inside MT5's Strategy Tester and was very likely failing every tick, structurally identical in effect to the spread bug. Fixed by skipping the staleness check specifically when running inside the Strategy Tester (`MQLInfoInteger(MQL_TESTER)`) — it's a live-feed-health concept that doesn't apply to a deterministic backtest replay. **Confirmed the same day**: founder re-ran GBPUSD M1 (2026.01.01–2026.07.15) and the History tab shows real trades with entries, TP/SL fills, and mixed win/loss outcomes — both fixes together resolved it. See `2026-07-16b_stale-quote-check-skipped-in-tester.md`.
**Progress note (2026-07-16c):** founder ran the equivalent XAUUSD M1 backtest on the same build and shared the actual `.xlsx` report (parsed directly, not read from a screenshot) — confirmed **0 trades, 0 deals** over 536,861 real bars. Cause: `MaxAllowedSpread_Points` (60) was shared by both allowed symbols but only tuned against GBPUSD; gold's normal spread runs much wider, as this project's own code comments already warned. Split into a separate `MaxAllowedSpread_Points_Gold` (200, new `IsGoldSymbol()` helper) — explicitly an unvalidated headroom estimate, not a confirmed real spread value. See `2026-07-16c_gold-needs-its-own-spread-limit.md`. Not yet re-confirmed on gold. Next real question across both symbols: does the strategy have an actual edge, not just does it place trades — need the full backtest Report (win rate, profit factor, drawdown).
...
- **EA core (MQL5):** The trading algorithm itself, written in MQL5 for MetaTrader 5. Includes:
  ...
  - Dual-mode profit-lock exit logic: (a) outright close, or (b) move stop to breakeven and then **trail it further behind price as profit grows** (2026-07-15 — previously locked breakeven once and stopped). Both exit modes configurable/testable. **Target dollar amount now differs by trading mode — see Trading Modes below** (Aggressive Mode: percentage of equity; Safe Mode: $0.30 flat, both tiers, 2026-07-14zz). A **maximum trade duration** (default 15 min, 2026-07-15) force-closes any position that's still open without hitting target/stop after that long, regardless of exit mode.
...
- **Trading Modes:**
  - **Safe Mode (redesigned 2026-07-14m, profit target changed again 2026-07-14zz):** Own profit-lock target, distinct from Aggressive Mode. Originally raised to **$1.50 at the <$50 tier (1:1.5 R:R, 40% break-even), $3.00 at the ≥$50 tier (1:1 R:R, 50% break-even)** in session 13 specifically to fix a punishing risk:reward ratio (see `2026-07-14m`) — **then lowered back down to a flat $0.30 on both tiers (2026-07-14zz)** per direct founder instruction for tighter XAUUSD/GBPUSD scalping, which reintroduces essentially the same problem session 13 fixed: break-even win rate is now **~77% (<$50 tier) / ~91% (≥$50 tier)**, both above the 65% win-probability filter still in place (see below) — documented and implemented as instructed, not silently reverted or blocked. Win-probability filter itself unchanged from its 2026-07-14m redesign: lowered from the original 80–100% to a realistic **65–75%** floor, though that floor was calibrated against the $1.50/$3.00 targets, not the new $0.30 one. A Monte Carlo simulation (`Product_Development/simulations/safe_mode_expectancy_simulation.py`) confirms positive expectancy across 50–75% assumed win rates **at the old $1.50/$3.00 targets** — **not re-run against the new $0.30 target; treat its conclusions as stale until it is.** Internal daily target: **5%** (2026-07-14l).
  - **Aggressive Mode:** Was no win-probability filter; **changed 2026-07-14v** to take any signal the confidence heuristic rates above **50%** (barely better than a coin flip) rather than literally everything — still the loosest filter of the two modes. **Redesigned 2026-07-14y:** no longer shares Safe Mode's tiered fixed-dollar stop-loss/target — instead risks and targets a **configurable percentage of current equity per trade (15%/15% default, 1:1)**, with its own, much higher **50% daily loss ceiling** (separate from Safe Mode's 3%, so the shared limit doesn't silently cap every Aggressive trade back down after one loss). A Monte Carlo risk-of-ruin simulation shows this is very safe (~0% ruin probability, "ruin" = equity falling to $10 from $100) if the strategy's real win rate matches the hoped-for 80%, but carries real risk (roughly 2–32% ruin probability) if the real win rate turns out closer to 50–55% — see `2026-07-14y_scalper1-name-and-aggressive-equity-risk.md` for the full numbers. Internal daily *profit* target unchanged: **20%** (2026-07-14k — top of the research-recommended 5–20% range). "Turn $50 into $1,000/day" / "$10 into $100/day" framing remains retired — **not engineered toward via compounding/martingale lot-sizing, and never referenced in marketing or the MQL5 Market listing.** Now the **default mode** on a fresh install (2026-07-14v).
  - Mode selection is user-configurable (a setting the customer picks, not something AITrader decides for them).
  - **Operation mode (2026-07-14x):** independent of Safe/Aggressive, the customer also picks `AUTO_TRADE` (bot places trades itself) or `SIGNALS_ONLY` (bot only alerts the user with a suggested trade — direction, size, stop-loss, take-profit — and never calls the trade API itself).
- **Daily Risk Controls:**
  ...
  - **Max 20 concurrent open trades** at any time (raised from 2, 2026-07-14zzz, per direct founder instruction — this multiplies simultaneous risk exposure, not just opportunity capture) — applies regardless of mode. Paired with a formal per-candle signal-stacking control (`AllowMultipleTradesPerSignal_Enabled`/`MaxTradesPerSignal_PerBar`) that makes explicit a behavior the code already had accidentally: the entry signal is re-checked every tick, not once per bar, so multiple trades could already open off one candle's signal within moments of each other.
  - **Daily loss limit: 3% of the day's starting equity for Safe Mode; 50% for Aggressive Mode (2026-07-14y, separate limits).** Once daily realized + floating losses reach the applicable limit, the EA stops trading until the next day. Aggressive Mode needs a much higher ceiling because a single Aggressive trade already risks 15% of equity by design — a shared 3% limit would silently cap it down to near nothing after the first loss. ⚠️ Flagged interaction (Safe Mode only): at exactly $50 equity, 3% = $1.50/day, but the ≥$50 stop-loss tier is $3/trade — meaning a **single** losing trade at that boundary exceeds the entire daily loss budget. Needs explicit handling (e.g., don't let a single trade's max loss exceed the remaining daily loss budget) rather than assuming the two limits compose cleanly.
...
- [x] Build Safe Mode's own exit logic (2026-07-14m targets) and a **rule-based heuristic** win-probability filter (re-derived for v2's mean-reversion signal, 2026-07-14u) — real, backtest-validated calibration still needed (currently an explainable but unvalidated scoring rule)
- [x] Build Aggressive Mode (now gated on >50% confidence, 2026-07-14v; was no filter) — explicitly without any compounding/martingale lot-sizing logic aimed at a daily multiplier target
...
- [x] **Design the real entry-signal logic** — v1 (2026-07-14q, trend+pullback) superseded by v2 (2026-07-14u): Bollinger Bands + RSI + Stochastic mean-reversion scalping, restricted to forex/metals. Not yet backtested — this is a hypothesis to validate, not a proven edge. See `decisions-learnings/2026-07-14u_scalping_signal_v2.md`.
- [ ] Compile the draft in MetaEditor and fix any syntax errors (not done — no MT5 environment available during staging)
- [ ] Run real MT5 Strategy Tester backtests — per mode, per stop-loss tier, across volatility regimes and major news events — to confirm actual win rate clears the break-even bars (40%/50% Safe Mode, 66.7%/85.7% Aggressive Mode). **A first real backtest attempt happened 2026-07-16 (GBPUSD M1) — it produced 0 trades due to a spread-filter bug, now fixed but not yet re-run**; still no data on actual win rate/drawdown/profitability. See `2026-07-16_spread-filter-blocking-all-trades.md`.
