# How to Compile AITrader.mq5 in MetaEditor — Step by Step

**Before you start:** `AITrader.mq5` requires **MetaTrader 5**, not MT4. They use different languages (MQL5 vs MQL4) and MT4 cannot open or run this file. If you only have MT4 installed (e.g., "FxPro MT4"), download MT5 separately — Exness offers both MT4 and MT5 installers on their site; make sure you get the MT5 one.
...
4. Copy `AITrader.mq5` into this `Experts` folder. (You can drop it directly in the folder, or in a subfolder like `Experts\AITrader\` if you want to keep it organized — either works.)
...
2. Navigate to the `Experts` folder from Step 2 and select `AITrader.mq5`.
...
- **If you see "0 error(s), 0 warning(s)"** (or just 0 errors, warnings are usually fine) — you're done. A new file `AITrader.ex5` now sits in the same `Experts` folder. That's the compiled version MT5 actually runs, and what eventually goes to MQL5 Market.
...
2. Expand **Expert Advisors** — you should see `AITrader` listed.
