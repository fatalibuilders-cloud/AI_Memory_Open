...
**A real Strategy Tester backtest has now been run twice — zero trades both times — with two separate blockers found and fixed, neither yet confirmed by a re-run.** Founder backtested GBPUSD M1 in MT5's Strategy Tester; a shared screenshot/video showed a flat 50-point simulated spread the whole test, which the EA's old 30-point spread filter rejected on every tick (`2026-07-16`, fixed: raised to 60). Re-ran, still zero trades — a second check in the same function (quote staleness, `SYMBOL_TIME`) is known to behave unreliably inside the Strategy Tester and was very likely also failing every tick; now skipped in tester mode (`2026-07-16b`). **Neither fix has been confirmed by an actual successful backtest yet, and neither was confirmed via the Experts/Journal log** — if trades still don't appear after this second fix, that log (not another guess) is the required next step. See both decision files for the reasoning.

## Other Open Items

1. **Re-run the GBPUSD M1 Strategy Tester backtest with both fixes (spread filter 60, staleness check skipped in tester)** and confirm trades now appear — if not, check the Experts/Journal log tab and share the exact block-reason text it prints, rather than guessing a third time. See `2026-07-16` and `2026-07-16b`.
