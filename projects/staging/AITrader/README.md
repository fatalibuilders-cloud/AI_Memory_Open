...
**The zero-trades bug is fixed and confirmed — the EA now places real trades in the Strategy Tester.** Two separate blockers were found and fixed in the same filter function: a fixed 50-point simulated spread exceeding the old 30-point cap (`2026-07-16`), and a quote-staleness check that misbehaves inside the Strategy Tester, now skipped there (`2026-07-16b`). Founder re-ran GBPUSD M1 (2026.01.01–2026.07.15) and the History tab shows real entries, stop-loss/take-profit fills, and mixed win/loss outcomes. **What's still missing: the full backtest Report (net profit, win rate, profit factor, max drawdown, total trade count).** A handful of visible trades says the plumbing works, not that the strategy has an edge — that's the actual open question now.

## Other Open Items

1. **Get the full backtest Report/summary** (net profit, win rate, profit factor, max drawdown, total trades) from the completed GBPUSD M1 run, and compare actual win rate against the documented break-even bars. Then repeat on XAUUSD and a longer/different date range before drawing any conclusions from one short window. See `2026-07-16b`.
