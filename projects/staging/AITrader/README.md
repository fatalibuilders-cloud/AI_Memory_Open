...
1c. **Decide if Safe Mode's profit target should actually drop toward $0.30** — the founder's "$0.30 profit" mention wasn't implemented as a new default, since it would undo the risk:reward fix from session 13; inputs already support it if genuinely wanted.
