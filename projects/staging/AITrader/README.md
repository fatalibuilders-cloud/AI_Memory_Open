...
**GBPUSD confirmed working; XAUUSD confirmed still broken, separately fixed, not yet re-confirmed.** GBPUSD M1 backtest (2026.01.01–2026.07.15) shows real trades with mixed win/loss outcomes — two blockers fixed (`2026-07-16`, `2026-07-16b`). The equivalent XAUUSD M1 backtest, same build, produced **0 trades / 0 deals** (confirmed from the actual `.xlsx` report, not a screenshot) — gold's spread was hitting the same 60-point cap tuned only for GBPUSD. Split into a separate `MaxAllowedSpread_Points_Gold` (200, unvalidated) — see `2026-07-16c`. **Still missing for both symbols: the full backtest Report** (net profit, win rate, profit factor, max drawdown, total trade count) — a handful of visible trades confirms the plumbing works, not that the strategy has an edge.

## Other Open Items

1. **Recompile and re-run the XAUUSD M1 backtest** to confirm the new gold-specific spread limit actually fixes it, the same way GBPUSD was confirmed. Then get the full backtest Report/summary (net profit, win rate, profit factor, max drawdown, total trades) for both symbols, and compare actual win rate against the documented break-even bars, before drawing any conclusions. See `2026-07-16c`.
