...
1i. **[Human] Validate the trailing-stop and max-trade-duration defaults** ($0.20 trail / $0.05 step / 15-minute limit) once real backtesting is possible — a 15-minute force-close could cut off trades that would have hit target shortly after, and force-closing at market can realize a loss larger than the configured stop if price has moved past it without triggering. See `2026-07-15`.
...
- **Trailing stop + maximum trade duration added.** Founder asked a second time to combine the uploaded `.ex5` files — declined again for the same reasons, but the underlying "make it more capable" goal was pursued with two independent, well-documented scalping techniques: `BREAKEVEN_AND_RUN` mode now trails the stop further as profit grows instead of locking flat at breakeven, and any position open longer than `MaxTradeDuration_Minutes` (default 15) force-closes regardless of exit mode. See `2026-07-15`.
