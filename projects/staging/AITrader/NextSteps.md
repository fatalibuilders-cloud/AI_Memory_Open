...
1. **[Dev] Recompile `FatalibuildersTraderScalper1.mq5` in MetaEditor** — renamed from `FatalibuildersTrader.mq5`; v0.90 changes added (2026-07-14x: plain-English input rename/groups + AUTO_TRADE/SIGNALS_ONLY manual mode; 2026-07-14y: "Scalper 1" name + Aggressive Mode's equity-percentage risk/reward and its own daily loss ceiling; 2026-07-14z: narrowed to XAUUSD/GBPUSD, M1 enforced, session-open delay filter), needs a fresh compile check.
...
1d. **[Human] Decide whether Safe Mode's profit target should actually be lowered toward $0.30** for XAUUSD/GBPUSD scalping, or left at the existing $1.50/$3.00 tiers — the founder's "even if it's a profit of $0.30" request was read as "accept small wins," not literally implemented as a new default, since it would reintroduce a risk:reward problem the $1.50/$3.00 targets were specifically designed to fix (2026-07-14m). The inputs already support $0.30 if that's the actual intent. See `2026-07-14z`.
2. **[Dev] Run the draft through MT5's Strategy Tester on XAUUSD and GBPUSD at M1** (both now required, not just recommended) — now with the v2 Bollinger/RSI/Stochastic scalping signal instead of the retired v1 trend+pullback. Check actual win rate against the documented break-even bars, not just that the plumbing works.
3. **[Human] Decide whether `MaxTradesOpenAtOnce` (2) should also be raised** as part of "more aggressive" — deliberately not changed in 2026-07-14v since it's a separate risk-exposure decision, not an opportunity-capture one.
...
- **Narrowed to XAUUSD and GBPUSD only, M1 chart enforced, session-open delay filter added.** `IsAllowedInstrument()` now only accepts gold and GBPUSD; `OnInit()` refuses to run on any timeframe other than M1; a new filter blocks new trades for 60 minutes (configurable) after each of the Asia/London/New York session opens (server time, needs verifying against the actual broker). Did not hardcode a $0.30 profit target — flagged as an open question (Priority Queue 1d) rather than silently changing Safe Mode's tuned defaults. See `2026-07-14z`.
