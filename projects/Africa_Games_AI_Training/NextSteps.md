...
2. **Playtest on real phones in Nairobi** — with both generations. Three things automated tests cannot judge: whether each **call is recognisable as that animal** (audibility is now measured, recognisability cannot be), whether the calls **grate** over a long session, and a **live duel between two phones on mobile data**.
...
5. **Play the difficulty curve** — 51 stages are generated by formula and monotonic by construction, but unplayed. Per-stage fail rates in soft launch are the only real validation.
...
8. **Blockers and new goal types** — snares to cut, waterholes to fill, an animal to escort down the board. This is what keeps a match-3 alive past level 30.
...
| Item | Date | Notes |
|---|---|---|
| Nairobi Wild v0.3.1 — animal calls rebuilt (harmonics + formants), music ducking, tappable voice legend, presence heartbeat | 2026-08-30 | 73/73 tests; calls measured by offline render, not by node counting |
