...
4. **Publish to Google Play.** The app itself is built — `android/` produces the `.aab`, and GitHub Actions builds it on every push (Actions → *Nairobi Wild — Android* → Artifacts). What is left is all yours, in this order, because the middle step takes two weeks of calendar time no matter how fast you work:
